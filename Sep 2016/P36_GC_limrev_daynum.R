...
P36_GC_limrev$medtreatday <- as.factor (paste(P36_GC_limrev$medtreat, P36_GC_limrev$day, sep="-"))

#make day numeric
P36_GC_limrev$daynum <- (as.numeric(P36_GC_limrev$day))-1

#delete day6 for phosphate
P36_GC_limrev <- P36_GC_limrev [! P36_GC_limrev$medtreatday=="-PO4-limitation-day6", ]

#plotting
qplot(as.factor(daynum), cellsml, data = P36_GC_limrev,  geom = "boxplot") + facet_grid(medtreat~., scales="free") 
qplot(as.factor(daynum), lncellsR, data = P36_GC_limrev,  geom = "boxplot") + facet_grid(medtreat~., scales="free") 

source("summarySE.R")
GC.sum <- summarySE(P36_GC_limrev, measurevar="lncellsR", groupvars=c("medtreat", "daynum"))

#plot with log normal cells R
ggplot(data=GC.sum, aes(x=daynum, y=lncellsR)) + geom_point(size=5)+ 
...
GC.sum.raw <- summarySE(P36_GC_limrev, measurevar="cellsml", groupvars=c("medtreat", "daynum"))

ggplot(data=GC.sum.raw, aes(x=daynum, y=cellsml)) + geom_point(size=5)+ 
...
expGC=as.data.frame(data.table(cbind(medtreat=P36_GC_limrev$medtreat, day=P36_GC_limrev$daynum, ID=P36_GC_limrev$medtreatreplet)))
...
levene.test(P36_GC_limrev$lncellsR, group=P36_GC_limrev$daynum, location="mean") # not significant
...
boxplot (lncellsR~daynum, data=P36_GC_limrev)


#fit a gls
Form <- formula (lncellsR ~ medtreat*daynum)
...
P36_GC_limrev7.lme <- lme (Form, random = ~1|medtreatreplet, correlation=corAR1(), method="REML", data=P36_GC_limrev) 
...
plot(x=P36_GC_limrev$daynum,y=P36_GC_limrev.E2,main="Time",ylab=MyYlab,xlab="Day")
par(op)

xyplot (P36_GC_limrev.E2 ~ daynum| medtreat, data=P36_GC_limrev, ylab="Residuals", xlab="Day", 
        panel=function(x,y){
          panel.grid(h=-1, v= 2)
