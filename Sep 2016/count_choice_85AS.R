...
mf_labeller <- function(var, value){
  ...
  if (var=="induction") { 
    value[value=="induced"] <- "induced"
    value[value=="notinduced"]   <- "not induced"
  }
  return(value)
}
...
allind.sum$bead2 <- factor(allind.sum$bead, levels=c("DPRbead", "dSibead"), labels =c ("Diproline bead", "dSi bead"))
allind.fitdata$bead2 <-  factor(allind.fitdata$bead, levels=c("DPRbead", "dSibead"), labels =c ("Diproline bead", "dSi bead"))


#bw
resize.win(6,9)

ggplot(data=allind.sum, aes(x=T, y=cellsBase, shape=bead2)) + geom_point(size=5)+ 
  ...
  facet_grid(induction~., scales="free", labeller=mf_labeller) +
